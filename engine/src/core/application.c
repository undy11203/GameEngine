...
b8 application_create(game* game_inst) {
    ...
    if(!event_initialize()) {
        UERROR("Event system failed initialization. Application cannot continue.");
        return FALSE;
    }
    ...
    if (!renderer_initialize(game_inst->app_config.name, &app_state.platform)) {
        UFATAL("Failed to initialize renderer. Aborting application.");
        return FALSE;
    }
...
b8 application_on_key(u16 code, void* sender, void* listener_inst, event_context context) {
    if (code == EVENT_CODE_KEY_PRESSED) {
        ...
        if (key_code == KEY_ESCAPE) {
            ...
            UDEBUG("Explicit - A key pressed!");
        } else {
            UDEBUG("'%c' key pressed in window.", key_code);
        }
    } else if (code == EVENT_CODE_KEY_RELEASED) {
        u16 key_code = context.data.u16[0];
        if (key_code == KEY_B) {
            // Example on checking for a key
            UDEBUG("Explicit - B key released!");
        } else {
            UDEBUG("'%c' key released in window.", key_code);
        }
    }
    return FALSE;
}