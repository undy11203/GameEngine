...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name, struct platform_state* plat_state) {
    ...
    app_info.pEngineName = "Undy Engine";
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
#if defined(_DEBUG)
    UINFO("Validation layers enabled. Enumerating...");
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        UINFO("Searching for layer: %s...", required_validation_layer_names[i]);
        b8 found = FALSE;
        ...
        if (!found) {
            UFATAL("Required validation layer is missing: %s", required_validation_layer_names[i]);
            return FALSE;
        }
    }

    UINFO("All required validation layers are present.");
#endif
    ...
    UINFO("Vulkan Instance created.");

    // Debugger
#if defined(_DEBUG)
    UDEBUG("Creating Vulkan debugger...");
    ...
    UASSERT_MSG(func, "Failed to create debug messenger!");
    VK_CHECK(func(context.instance, &debug_create_info, context.allocator, &context.debug_messenger));
    UDEBUG("Vulkan debugger created.");
#endif

    // Surface
    UDEBUG("Creating Vulkan surface...");
    if (!platform_create_vulkan_surface(plat_state, &context)) {
        UERROR("Failed to create platform surface!");
        return FALSE;
    }
    UDEBUG("Vulkan surface created.");

    // Device creation
    if (!vulkan_device_create(&context)) {
        UERROR("Failed to create device!");
        return FALSE;
    }
    ...
    UINFO("Vulkan renderer initialized successfully.");
    return TRUE;
}
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend) {
    ...
    UDEBUG("Destroying Vulkan device...");
...
void create_command_buffers(renderer_backend* backend) {
    if (!context.graphics_command_buffers) {
        ...
        for (u32 i = 0; i < context.swapchain.image_count; ++i) {
            uzero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
        }
    }
   ...
   UDEBUG("Vulkan command buffers created.");
}
