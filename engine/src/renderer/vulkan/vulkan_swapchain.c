...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        UFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        UFATAL("Failed to present swap chain image!");
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    swapchain_extent.width = UCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = UCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)uallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)uallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        UFATAL("Failed to find a supported format!");
    }
    ...
    UINFO("Swapchain created successfully.");
}
